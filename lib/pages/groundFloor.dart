...
import 'package:washio/pages/booking_zero.dart';
import 'package:washio/pages/booking_zero_next.dart';
    ...
    for (var row in data) {
      ...
      try {
        ...
        if (slotTimes.length == 2) {
          ...
          // Update status in Supabase if it has changed
          if (newStatus != currentStatus) {
            await Supabase.instance.client.from('floor0_nextday');
            //.update({'Status': newStatus}).eq('Name', row['Name']);
          }
        }
      } catch (e) {
        ...
        onPressed: () {
          if (isNextDay) {
            Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => const BookingPageZeroNext()),
            );
          } else {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => const BookingPageZero()),
            );
          }
        },
        backgroundColor: const Color.fromARGB(255, 0, 0, 0),
